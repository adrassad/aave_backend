...
export function startCommand(bot) {
  const ruCommands = [
    ...
    { command: "set_threshold", description: "🎯 Установить порог HF" },
    { command: "help", description: "❓ Показать все команды" },
    { command: "support", description: "💬 Написать в поддержку" },
  ];
  bot.telegram.setMyCommands(ruCommands, { language_code: "ru" });
  ...
  const enCommands = [
    ...
    { command: "set_threshold", description: "🎯 Set HF alert threshold" },
    { command: "help", description: "❓ Show all commands" },
    { command: "support", description: "💬 Contact support" },
  ];
  bot.telegram.setMyCommands(enCommands, { language_code: "en" });
  bot.telegram.setMyCommands(enCommands);
  bot.start(async (ctx) => {
